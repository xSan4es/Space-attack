...
public class GameControllerLVL3 : MonoBehaviour, IGameController
{
    ...
    IEnumerator SpawnEnemies()
    {
        ...
        while (true)
        {
            while (true)
            {
                ...
                if (ii % 11 == 0)
                {
                    randDistance_x = Random.Range(0, 6);
...
public void end_game()
    ...
    {
        return new Vector3(
                    SearchVariable.portation.transform.position.x,
                    SearchVariable.portation.transform.position.y + 3 * Random.Range(0, 7) * camera.ScreenToWorldPoint(new Vector3(Screen.width / 1.1f - SearchVariable.portation.transform.position.y, 0, 0)).y / 14,
                    0);
    }
